# Generic Interface for Generic Accelerators (v1.0.0)
...
This is an implicit operation which consists in reinterpreting tensor data with a different shape. It is done without copy or memory overhead.
...
The API offers the possibility to have all the processing done in an asynchronous manner. A callback and wait-for-completion mechanism allows resynchronisation after processing. This is particularly useful for very multi-threaded environments or to avoid wasting CPU cycles waiting for an accelerator to finish its work.

### Helper scripts

Several Python scripts are provided as tools to cleanup, export and convert neural networks from ONNX and NNEF formats. These scripts can generate C code relying on GIGA to automatize deployment.
